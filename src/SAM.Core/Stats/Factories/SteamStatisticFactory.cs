...
public static class SteamStatisticFactory
{
    ...
    {
        if (string.IsNullOrEmpty(infoBase?.Id)) throw new ArgumentNullException(nameof(infoBase));
        ...
        switch (infoBase)
        {
            ...
            {
                if (!client.SteamUserStats.GetStatValue(infoBase.Id, out int value)) break;
